...
const check = () => {
  ...
  win.forEach((e) => {
    if (
      ...
      document.querySelector(".info").innerHTML =
        box_text[e[0]].innerHTML + " YOU WON!!";
      isbackground = true;
  ...
  if (!isbackground) {
    ...
    if (isTie) {
      setTimeout(() => {
        if (confirm("Evenly matched, time for a rematch!")) {
          resetGame();
        }
      }, 2000);
    }
...
Array.from(boxes).forEach((element) => {
  ...
  element.addEventListener("click", () => {
    if (!isGameOver && box_text.innerHTML === "") {
      ...
      if (!isbackground && !isGameOver) {
        if (turn === "❌") {
            document.querySelector(".info").innerHTML = "Go forward, Player 1!";
        } else {
            document.querySelector(".info").innerHTML = "Go forward, Player 2!";
        }
    }
